...
//Read Json and parse
const jsonData = fs.readFileSync('Box.json', 'utf8');
const boxdata = JSON.parse(jsonData);
//data to JSON
const data = boxdata;
...
function pause() { // Pause Program | Press any key to contine 
...
async function store() {  //Add a Pokemon
  ...
  if (Number(poke)) {                                          // Check if Number | Catch fetch error
    ...
    try {
      ...
      return console.log("An error was caught");
    }
  }
  pause();
}
  ...
  if (Number(poke)) {                                          // Check if Number | Catch fetch error
    ...
    try {
      ...
      app.get(`/meow.html/${poke}`, async (req, res) => {
        ...
        <style>body {  background-image: url('https://c4.wallpaperflare.com/wallpaper/996/1000/959/pokemon-wallpaper-preview.jpg');}
                #wrap {width: 50%; margin: auto;}
                  img.animated-gif{
                width: 300pxx;
                height: 400px;
              }
              p  { background-color: rgba(218, 247, 166, 0.6); text-align: center; /* YellowGreen */}
              h1 { background-color: rgba(255, 255, 255, 0.7); text-align: center; /* or any other color you prefer */}
              h2 { background-color: rgba(255, 255, 255, 0.5); padding: 10px;/* or any other color you prefer */}
                </style>
        </head>
        <body> 
              ...
              <div width: 75%;  border: 10px solid #4CAF50; style="text-align: center; background-color: rgba(255, 255, 255, 0.2); padding: 10px;">
               <img class="animated-gif" src="${image}"/>
                </div>
              ...
              <h2>Moves:</h2> 
              <p>${item.moves.join(', ').toUpperCase()}</p>
                 </div>
              </div>
      </body>
      </html>
